...
module ArticlesHelper
  ...
  def article_status_badge(article)
    if article.published?
      content_tag :span, class: 'badge badge-success' do
        concat(content_tag(:svg, class: 'badge-icon', fill: 'none', stroke: 'currentColor', viewBox: '0 0 24 24') do
  ...
  def article_card_class(article)
    ...
    classes << 'article-card--featured' if article.respond_to?(:featured?) && article.featured?
    classes.join(' ')
  end
  ...
  def article_meta_tags(article)
    ...
    # 基本的なメタタグ
    tags << tag.meta(name: 'description', content: article_summary(article, length: 160))
    tags << tag.meta(name: 'author', content: article.user.name)
    
    # Open Graph タグ
    tags << tag.meta(property: 'og:title', content: article.title)
    tags << tag.meta(property: 'og:description', content: article_summary(article, length: 160))
    ...
    tags << tag.meta(name: 'twitter:description', content: article_summary(article, length: 160))
  ...
  def article_navigation(article)
    {
      prev: Article.where('created_at < ?', article.created_at).order(created_at: :desc).first,
      next: Article.where('created_at > ?', article.created_at).order(created_at: :asc).first
    }
  end
end
